...
void MuPdfBridge::processFonts(CmdRequest& request, CmdResponse& response)
{
    ...
    while (iter.hasNext())
    {
        ...
        case SANS:
            ...
            if (!iter.isValid())
            {
                LE("No SANS fonts found");
                response.result = RES_BAD_REQ_DATA;
                return;
            }
            LI("Regular     : %s", fonts[REGULAR]);
            LI("Italic      : %s", fonts[ITALIC]);
            LI("Bold        : %s", fonts[BOLD]);
            LI("Bold Italic : %s", fonts[BOLD_ITALIC]);
        ...
        case DINGBAT:
            ...
            break;
        default:
            LE("Unknown font family: %d", fontFamily);
            response.result = RES_BAD_REQ_DATA;
            break;
        }
    }
...
static void gatherfonts(fz_context *ctx, pdf_document* doc, pdf_obj* rsrc, pdf_obj *dict, std::set<std::string>& external, std::set<std::string>& all)
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        if (font)
        {
            ...
            {
                LI("Built-in Document font: basefont=%s file=%s", basefontname.c_str(), font->font->ft_filepath);
            }
            else if (font->external_origin == PDF_FD0_SYSTEM)
            {
                LI("System   Document font: basefont=%s file=%s", basefontname.c_str(), font->font->ft_filepath);
            }
            else
            {
                LI("External Document font: basefont=%s file=%s", basefontname.c_str(), font->font->ft_filepath);
